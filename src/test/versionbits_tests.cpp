...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow, nTime, ALGO_RANDOMX).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit), 0);

    // Mine more blocks (4 less than the adjustment period) at the old time, and check that CBV isn't setting the bit yet.
    for (uint32_t i = 1; i < mainnetParams.nMinerConfirmationWindow - 4; i++) {
        lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow + i, nTime, ALGO_RANDOMX).Tip();
        // This works because ALGO_RANDOMX happens
    ...
    lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow * 3, nTime, ALGO_RANDOMX).Tip();
    // so ComputeBlockVersion should now set the bit,
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit)) != 0);
    // and should also be using the VERSIONBITS_TOP_BITS.
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
    ...
    while (nTime < nTimeout && blocksToMine > 0) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, ALGO_RANDOMX).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit)) != 0);
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
        blocksToMine--;
        nTime += 600;
    ...
    for (uint32_t i = nHeight; i < mainnetParams.nMinerConfirmationWindow * 5; i++) {
        lastBlock = firstChain.Mine(i, nTime, ALGO_RANDOMX).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit)) != 0);
    }

    // The next block should trigger no longer setting the bit.
    nHeight = mainnetParams.nMinerConfirmationWindow * 5;
    lastBlock = firstChain.Mine(nHeight, nTime, ALGO_RANDOMX).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit), 0);
    ...
    // next period.
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow, nTime, ALGO_RANDOMX).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit)) != 0);
    ...
    lastBlock = secondChain.Mine((mainnetParams.nMinerConfirmationWindow * 3) - 1, nTime, ALGO_RANDOMX).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1 << bit)) != 0);
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow * 3, nTime, ALGO_RANDOMX).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams, ALGO_RANDOMX) & (1<<bit), 0);
