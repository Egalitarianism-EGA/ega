...
static void TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst) EXCLUSIVE_LOCKS_REQUIRED(::mempool.cs)
{
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX);
    ...
    pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX);
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX));
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX), std::runtime_error, HasReason("bad-blk-sigops"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX), std::runtime_error, HasReason("bad-txns-inputs-missingorspent"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX), std::runtime_error, HasReason("bad-cb-multiple"));
    mempool.clear();
    ...
    while (chainActive.Tip()->nHeight < 209999) {
        ...
        next->nVersion |= BLOCK_VERSION_RANDOMX;
        next->pprev = prev;
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, ALGO_RANDOMX), std::runtime_error, HasReason("block-validation-failed"));
    mempool.clear();
