...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            "getblocktemplate ( TemplateRequest ) ( algo )\n"
    ...
    // EGA fair launch: allow solo GBT with zero peers (new network).
    // Set -gbtrequirepeers=1 later when the network is established.
    if (gArgs.GetBoolArg("-gbtrequirepeers", false) &&
        g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EGA is not connected!");

    // Genesis tip age would keep IBD true forever until blocks are mined; do not
    // block mining templates on IBD during launch. Optional strict mode:
    if (gArgs.GetBoolArg("-gbtrequiresynced", false) && IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EGA is downloading blocks...");
    ...
    // EGA: No Odo key; dual algo only (RandomX/Verthash)
