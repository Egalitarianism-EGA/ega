...
{
    // uiInterface.NotifyBlockTip.connect(&RPCNotifyBlockChange); // EGA: commented for boost compat in this env; RPC block notify disabled temporarily
}

static void OnRPCStopped()
{
    // uiInterface.NotifyBlockTip.disconnect(&RPCNotifyBlockChange); // EGA temp comment
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-algo=<algo>", "Default mining algorithm: randomx | verthash | yespower-ega (default: randomx)", false, OptionsCategory::BLOCK_CREATION);
    gArgs.AddArg("-gbtrequirepeers", "Require P2P peers for getblocktemplate (default: 0; set 1 after network has peers)", false, OptionsCategory::BLOCK_CREATION);
    gArgs.AddArg("-gbtrequiresynced", "Require node out of IBD for getblocktemplate (default: 0 for fair launch)", false, OptionsCategory::BLOCK_CREATION);
...
/** Sanity checks
 *  Ensure that EGA is running in a usable environment with all
...
{
    // Make sure only a single EGA process is using the data directory.
...
bool AppInitMain()
{
    ...
    // Default mining algo for GBT / generate when not overridden per-call
    std::string strAlgo = gArgs.GetArg("-algo", "randomx");
    miningAlgo = GetAlgoByName(strAlgo, ALGO_RANDOMX);
    LogPrintf("Selected mining algo: %s (%s)\n", strAlgo, GetAlgoName(miningAlgo));
    ...
    if (chainActive.Tip() == nullptr) {
        // EGA: Temporarily disabled for Boost compatibility during build
        // uiInterface.NotifyBlockTip.connect(BlockNotifyGenesisWait);
        fHaveGenesis = true;  // Assume genesis is present for build
    } else {
        fHaveGenesis = true;
    }

    if (gArgs.IsArgSet("-blocknotify"))
        // uiInterface.NotifyBlockTip.connect(BlockNotifyCallback);  // EGA: temp disable
        ;  // no-op for now
    ...
    // Wait for genesis block to be processed
    {
        ...
        // uiInterface.NotifyBlockTip.disconnect(BlockNotifyGenesisWait);  // EGA temp disable
    }
