...
#if defined(NDEBUG)
# error "EGA cannot be compiled without assertions."
#endif
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    bool fOdoActive = false; // EGA: Odo logic removed; using dual RandomX/Verthash from genesis
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        {
            LOCK(cs_main);
            // EGA: No Odo fork; all peers start with current version
            if (false)
                minVersion = std::max(minVersion, 0);
        }
